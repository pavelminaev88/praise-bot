...
- **Memory:** your last 10 messages (and the replies) in a private chat; each is forgotten 24 h after it was written. `/forget`, `/mydata`, `/privacy`.
- **Feedback:** 🔥😐👎 under each praise, counted anonymously. `/stats` for the owner.
- **Weekly check:** every Saturday morning the bot tests itself (Telegram, storage, one real Claude reply) and sends the owner a report with weekly stats and API costs. `/selftest` runs it now.
...
| What | Where | How long |
|---|---|---|
| Last messages of a **private** chat | Workers KV, keyed by a one-way hash of the chat ID (no IDs, no names) | each message 24 h; the whole record is auto-deleted 24 h after the last message |
...
4. Open `https://<your-worker>.workers.dev/setup?avatar=1` once. It connects the bot to Telegram and sets its name, descriptions ([`src/profile.ts`](src/profile.ts)) and photo ([`public/avatar.jpg`](public/avatar.jpg)).
